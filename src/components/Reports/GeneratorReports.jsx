...
const GeneratorReports = () => {
    ...
    const renderChartWasteByLab = () => {
        ...
        const labsUser = usere.asigLaboratorios.map(as => as)
        console.log("LABSAS:",labsUser);
        const uniqueLaboratorios = [...new Set(labsUser.map(lab => lab.id))]
        ...
        const labelsLabs = labsUser.map(lab => lab.nombreLaboratorio)
        console.log(uniqueLaboratorios.map(m => wasteByStatement.filter(waste => waste.id_laboratorio === m).length));
        const arrData = uniqueLaboratorios.map(m => labs.find(lab => lab.id === m).cantidadGenerada)
        const data = {
            labels: labelsLabs,
            datasets: [
                {
        ...
        <Container sx={{ py: 4, pt: 1 }}>
            ...
            <Grid container gap={2} sx={{ mt: 2 }}>
                <Grid item container md={12} sx={{ justifyContent: "space-around" }}>
                    <Grid container item md={8} sx={{ p: 2 }} component={Card} elevation={5}>
                        <Grid item md={12}>
                            <Typography sx={{ textAlign: "center", fontWeight: "bold", fontSize: 25, mb: 4 }}> Cantidad de residuos(kg) generados por laboratorio</Typography>
                            <canvas id="chartWasteByLab"></canvas>
                        </Grid>
                    </Grid>
                    {/* <Grid container item md={5.5} sx={{ p: 2 }} component={Card} elevation={5}>
                        <Grid item md={12}>
                            <Typography sx={{ textAlign: "center", fontWeight: "bold", fontSize: 25, mb: 4 }}>Cantidad de residuos por facultad</Typography>
                            <canvas id="chartWasteByFacultad"></canvas>
                        </Grid>
                    </Grid> */}
                    <Grid container item md={11} sx={{ p: 2, mt: 4 }} component={Card} elevation={5}>
                        <Grid item md={12}>
                            <Typography sx={{ textAlign: "center", fontWeight: "bold", fontSize: 25, mb: 4 }}>Cantidad(kg) de residuos generados por periodo</Typography>
                        </Grid>
                    ...
                    <Grid container item md={8} sx={{ p: 2, mt:4 }} component={Card} elevation={5}>
                        <Grid item md={12}>
